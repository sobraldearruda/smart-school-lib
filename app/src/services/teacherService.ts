...
export class TeacherService {
  ...
  async getTeacherByRegistration(userRegistration: string): Promise<Teacher> {
    ...
    if (!teacher) throw new UserNotFoundException(`Teacher with registration ${userRegistration} not found.`);
    return teacher;
  }
  ...
  async updateTeacher(userRegistration: string, updatedData: Partial<Omit<Teacher, "userId">>): Promise<Teacher> {
    ...
    if (!existing) throw new UserNotFoundException(`Teacher with registration ${userRegistration} not found.`);
  ...
  async authenticate(userRegistration: string, userPassword: string) {
    ...
    if (!user) throw new UserNotFoundException(`Teacher with registration ${userRegistration} not found.`);
